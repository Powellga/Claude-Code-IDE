...
9. **WebSocket Events** - `start_terminal`, `resume_session`, `reattach_terminal`, `terminal_input`, `resize_terminal`, `stop_terminal`, `discard_terminal`. Every event carries a `terminal_id` (client-generated for start/resume); `_resolve_terminal_id()` validates ownership and falls back to the connection's only terminal when the field is missing (legacy frontend). Disconnect does NOT kill sessions: it orphans them (`sid = None`) with a grace timer (`CLAUDE_IDE_ORPHAN_GRACE`, default 90s). A reloading page fetches `GET /api/active-terminals` and reattaches via `reattach_terminal`, which cancels the reaper, rebinds the sid, replays the output tail, and the client sends a resize nudge to make the TUI repaint. Only when the grace expires does `_reap_orphan()` auto-save and kill. PTY reader threads resolve the terminal's CURRENT sid per emit (`_current_sid()`) - never capture the spawn-time sid. Session lifecycle invariants: a Claude process that exits on its own is finalized by the reader thread's `_finalize_exited_terminal()` (saves the record, removes the entry, emits `session_saved`) - stop/discard/reap pop the entry via `_kill_terminal()` BEFORE the process dies, so the identity check makes finalize a no-op for them (discards stay discarded). `_spawn_terminal()` kills and saves any existing entry under a reused term_id before spawning (never overwrite - the old PTY would interleave output into the new tab and its record would be lost). Stop/discard resolve with `allow_orphaned=True` so they work right after a reconnect; a stop with nothing to save emits a `terminal_error` instead of silence. Client reconnects REATTACH first and only fall back to `resume_session` when the server answers `code: "not_running"` (`sess.resumeFallback` in app.js) - resuming while the orphaned terminal is still alive gets refused as already-open and the session would die at grace expiry
10. **Usage API** - `/api/usage` parses Claude Code's own jsonl transcripts matched to IDE sessions - scans `~/.claude/projects` PLUS every configured account's `<config_dir>/projects`. Streamed responses repeat identical usage per message id - dedupe by id before summing; tokens are bucketed PER MODEL so estimated cost can be priced from `DEFAULT_MODEL_PRICING` (prefix-matched on model id, overridable via `model_pricing` in settings.json). Cached by mtime+size in `data/usage_cache.json`; entries without a `models` key are the old flat format and re-parse automatically
11. **Accounts** - settings.json `accounts: [{name, config_dir, api_key}]`. `_account_env()` maps a name to `CLAUDE_CONFIG_DIR` / `ANTHROPIC_API_KEY` overrides merged into the PTY env at spawn. Each session records its `account`; resume MUST use the session's original account (the jsonl transcript lives in that account's config dir), which the frontend passes from the saved record - never the current selector
...
- Multi-session tabs: `termSessions` maps `terminal_id` -> session object (own xterm instance, container div, tab element, project, running flag). `activeSess()` returns the active tab's session; toolbar buttons and upload/screenshot/import prompts always target the active tab. A tab captures its project from the sidebar at spawn time - changing the sidebar later never affects a running tab. Never reintroduce a global `terminal`/`isTerminalRunning` singleton. Tab labels are the session UUID (short form) - deliberately, several tabs on one project are otherwise indistinguishable; project/account/full UUID live in the tooltip
- Duplicate-session protection: `resumeInTab()` switches to an existing tab holding that claude_session_id (or reuses an idle one), and the server refuses `resume_session` for an id already live in `active_terminals`. Both layers exist because multiple open IDE pages each auto-resume on server-restart reconnect - keep both
...
- **AutoSave** - Each time a code update is completed, commit and push to GitHub AUTOMATICALLY, without asking (one `git push origin master` reaches both accounts). Always update the README.md first when features or behavior changed (create it if it does not exist). Never end a session leaving completed work uncommitted; if something must stay unpushed, Gregg will say so explicitly.
...
- **Import-session now copies the transcript (2026-09-04):** Claude Code only finds a `--resume` transcript under `<config>/projects/<munged cwd>/` for the CURRENT cwd (munge = every non-alphanumeric char -> `-`, see `_claude_project_dirname()`). `/api/import-session` used to write a record pointing at the new workspace while the jsonl stayed under the original cwd's folder, so every import whose workdir differed from the original was unresumable. It now 404s if no transcript exists anywhere (`_find_transcript()` searches the default and every account's projects dir), copies the jsonl into the workdir's folder via `_place_transcript_for_workdir()` (never overwrites an existing copy; the original is left in place), and stores `account` on the record. Two pre-existing broken imports (macrium 9c68eed2, file-manager b4e87fca) were repaired by hand the same way.
- **No configured accounts (2026-09-23):** the unused "Temark" account entry (broken relative config_dir, never ran a session) was removed from settings.json and its Anthropic API key (`sk-ant-api03-SZS...IQAA`) was REVOKED in the console by Gregg. Copies of that key in old backup zips / backup-repo history are inert - do not flag them. All sessions run under the default ~/.claude login (gregg@temarkinternational.com, Temark org), so all token usage bills to Temark.
- **Backup git mirror excludes >95 MB files (2026-09-04):** GitHub rejects pushes whose history contains a 100 MB+ file; the pre-trim 368 MB temark-box session snapshot got committed to the backup repo (Claude-Code-IDE-Backups) on 2026-08-13 and silently blocked every push after. `backup.py` now strips oversized files from the mirror before committing (they stay in data/ and the local zips). History repaired and pushed 2026-09-04 (oversized blob swapped for the trimmed version in the 6 unpushed commits; pre-rewrite history kept on local branch `backup-pre-lfs-rewrite`, safe to delete once a few more backups have pushed).
