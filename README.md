...
<img width="2553" height="1408" alt="claude code IDE screenshot" src="https://github.com/user-attachments/assets/3082a17a-7141-4961-80da-d5427f23c495" />
...
### Self-Maintaining Project Context (CLAUDE.md)
Every project's CLAUDE.md carries a **Project Context Log** section - a standing instruction that tells Claude to keep a compact rolling summary of the project (decisions, architecture, external IDs, constraints, blockers) updated in the same session those facts change. Because Claude Code loads CLAUDE.md at the start of every session, each new or resumed session begins with the project's accumulated context, independent of conversation history. The section is included in the CLAUDE.md template for new projects, and the server self-heals it at session spawn: any project whose CLAUDE.md predates the feature (or was hand-written without it) gets the section appended the next time a session opens there. Existing content is never rewritten, and a hand-maintained equivalent section is left alone.
